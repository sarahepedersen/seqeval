...
def _sample_next_birth(
    age_yr: np.ndarray, parity: int, hazards: HazardSpec, horizon: float, rng: np.random.Generator
) -> np.ndarray:
    ...
    is over the handful of age bands.
    """
    n = len(age_yr)
...
def _simulate_births(
    ...
    Returns ``(row_index, birth_age_yr)`` for every birth. Vectorized: the outer loop is over
    parity levels (bounded by ``max_parity``); at level ``p`` only rows currently at parity ``p``
    and still fertile are sampled, so heterogeneous starting parities are handled in one pass.
    """
    ...
    for p in range(hazards.max_parity):
        ...
        nxt = _sample_next_birth(current[idx], p, hazards, horizon, rng)
        got = np.isfinite(nxt)
        fired = idx[got]
...
def simulate_cohort(
    ...
    # Trailing no-event markers: always for zero-birth persons, a random fraction otherwise.
    ...
    marker = np.where(has_birth, chosen, True) & (end_day > last_birth_day)

    marker_pid = person_id[marker]
    marker_days = np.full(marker_pid.shape, end_day, dtype=np.int32)
    ...
    persons = pd.DataFrame(
        {
            "person_id": person_id,
            "birth_year": rng.integers(birth_years[0], birth_years[1] + 1, size=n).astype(np.int16),
